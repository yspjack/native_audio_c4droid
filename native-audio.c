...
/* This is a JNI example where we use native methods to play sounds using
   OpenSL ES. See the corresponding Java source file located at:
...
#include <stdlib.h>
#define assert(expr)\
if(!(expr)){fprintf(stderr,"%s: %d: %s: Assertion `%s' failed\n",__FILE__,__LINE__,__func__,__STRING(expr));abort();}
...
void bqRecorderCallback(SLAndroidSimpleBufferQueueItf bq, void *context)
{
	...
	assert(NULL == context);
	// for streaming recording, here we would call Enqueue to give recorder
	// the next buffer to fill
...
void createEngine()
{
	...
	// the required MODIFY_AUDIO_SETTINGS permission was not requested and
	// granted
...
#if 0							// mute/solo is not supported for sources that
	// are known to be mono, as this is
...
bool createUriAudioPlayer(const char *uri)
{
	...
	result = (*engineEngine)->CreateAudioPlayer(engineEngine, &uriPlayerObject, &audioSrc,
												&audioSnk, 3, ids, req);
	// note that an invalid URI is not detected here, but during
	// prepare/prefetch on Android,
	...
	// get the mute/solo interface
	result =
		(*uriPlayerObject)->GetInterface(uriPlayerObject, SL_IID_MUTESOLO, &uriPlayerMuteSolo);
	assert(SL_RESULT_SUCCESS == result);
	(void)result;
...
void setLoopingUriAudioPlayer(bool isLooping)
{
	SLresult result;
...
void setChannelMuteUriAudioPlayer(int chan, bool mute)
{
	SLresult result;
...
void setChannelSoloUriAudioPlayer(int chan, bool solo)
{
	SLresult result;
	SLMuteSoloItf muteSoloItf = getMuteSolo();
	if (NULL != muteSoloItf)
...
void setMuteUriAudioPlayer(bool mute)
{
	SLresult result;
	SLVolumeItf volumeItf = getVolume();
	if (NULL != volumeItf)
...
void enableStereoPositionUriAudioPlayer(bool enable)
{
	SLresult result;
	SLVolumeItf volumeItf = getVolume();
	if (NULL != volumeItf)
...
bool enableReverb(bool enabled)
{
	...
	// we might not have been able to add environmental reverb to the output
	// mix
...
// select the desired clip and play count, and enqueue the first buffer if
// idle
bool selectClip(int which, int count)
{
	...
	if (nextSize > 0)
	{
		...
		// but for streaming playback we would typically enqueue at least 2
		// buffers to start
		SLresult result;
		result = (*bqPlayerBufferQueue)->Enqueue(bqPlayerBufferQueue, nextBuffer, nextSize);
		if (SL_RESULT_SUCCESS != result)
...
bool createAssetAudioPlayer(jobject assetManager, char *filename)
{
	SLresult result;

	// convert Java string to UTF-8
	const char *utf8 = filename;
	assert(NULL != utf8);
	int fd, start, length;
	fd = open(utf8, 0);
	...
	const SLboolean req[3] = { SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE, SL_BOOLEAN_TRUE };
	result =
		(*engineEngine)->CreateAudioPlayer(engineEngine, &fdPlayerObject, &audioSrc, &audioSnk, 3,
										   ids, req);
	assert(SL_RESULT_SUCCESS == result);
	(void)result;
...
void shutdown()
{
	...
	// destroy file descriptor audio player object, and invalidate all
	...
	// destroy URI audio player object, and invalidate all associated
	// interfaces
...
int main(int argc, char **argv)
{
	...
	while (!finished)
	{
		...
		case 'p':
			playing = !playing;
			setPlayingUriAudioPlayer(playing);
			puts(playing ? "Playing" : "Pause");
			break;
		case 'q':
			// default:
			finished = true;
